(function () {
  ...
  const NOTIFICATIONS = [
    { icon: "📢", title: "I moved jobs!", message: "Learn more about my new work in Resume.txt and AI Chat!" },
    { icon: "💾", title: "I just updated the pictures. Take a look!", message: "This is version 0.10.3." },
    { icon: "🔔", title: 'I updated the resume. Check it out in "My Computer"!', message: "This is version 0.10.2." },
  ];
  ...
  const VIDEOS = [
    ...
    { id: "7AxIA9tsjYA", title: "고민중독 (tbh) - qwer", description: "arrangement by LiNela. my confession." },
    ...
    { id: "fCECnGZXEbg", title: "봄여름가을겨울 (still life) - bigbang", description: "arranged by me, inspired by others." },
    { id: "NfF9dMV8DN0", title: "dilemma - nelly", description: "slowed and reverbed. arranged by torbjørn hoelsveen." },
  ];
  ...
  const COMPUTER_FOLDERS = {
    ...
    Writings: [
      ...
      { id: "writing2", name: "파란 돌.txt", type: "document", icon: "text", description: "A Korean poem by Han Kang." },
    ],
  ...
  const FILES = {
    ...
    writing2: { title: "파란 돌.txt", icon: "text" },
  };
  ...
  function nextWindowBounds(defaultWidth, defaultHeight) {
    ...
    return {
      ...
      height: Math.min(defaultHeight, window.innerHeight - 70),
    };
  }
  ...
  function showDialog(title, message) {
    ...
    backdrop.innerHTML = '<div class="dialog-window visible"><div class="dialog-titlebar"><div class="dialog-title">' + iconMarkup("computer", 16) + "<span>" + escapeHtml(title) + '</span></div><button class="dialog-close-btn" type="button">×</button></div><div class="dialog-content"><div class="dialog-icon">💬</div><div class="dialog-message">' + escapeHtml(message) + '</div></div><div class="dialog-buttons"><button class="dialog-btn primary" type="button">OK</button></div></div>';
  ...
  function openPrimaryWindow(type) {
    ...
    if (type === "mail") return createWindow({ id: "window-mail", type: "mail", title: "Mail", icon: "mail", width: 560, height: 480, data: { status: "" } });
  ...
  function attachWindowFrame(win) {
    ...
    titleBar.addEventListener("mousedown", function (event) {
      ...
      function onMove(moveEvent) {
        win.x = Math.max(0, Math.min(window.innerWidth - 160, originX + moveEvent.clientX - startX));
        win.y = Math.max(0, Math.min(window.innerHeight - 90, originY + moveEvent.clientY - startY));
        win.applyBounds();
      }
      function onUp() {
    ...
    resizeHandle.addEventListener("mousedown", function (event) {
      ...
      function onMove(moveEvent) {
        win.width = Math.max(320, Math.min(window.innerWidth - win.x - 4, originWidth + moveEvent.clientX - startX));
        win.height = Math.max(220, Math.min(window.innerHeight - win.y - 44, originHeight + moveEvent.clientY - startY));
        win.applyBounds();
      }
      function onUp() {
        window.removeEventListener("mousemove", onMove);
  ...
  function renderChatWindow() {
    return '<div class="msn-chat"><div class="msn-toolbar"><div class="msn-toolbar-section"></div></div><div class="msn-chat-container"><div class="msn-loading" data-chat-loading><div class="loading-spinner"></div><p>Connecting to AI chat...</p></div><iframe src="https://cofounder.co" class="msn-iframe" title="AI Chat" allow="microphone; camera" sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-modals"></iframe></div><div class="msn-input-area"><div class="msn-input-info"><span class="msn-status-icon">🟢</span><span class="msn-status-text">Connected to AI Agent</span></div><div class="msn-input-hint">💬 Chat with the AI agent in the window above</div></div><div class="msn-statusbar"><span>AI Chat Powered by cofounder.co</span><span class="msn-connection-status">● Online</span></div></div>';
  }

  function renderMailWindow(win) {
    return '<div class="mail-content"><div class="file-edit-container-mail"><p>File</p><p>Edit</p><p>Format</p><p>Help</p></div><div class="mail-form-wrapper"><form class="form-container" data-mail-form><div class="to-container"><div class="to-icon"><p>To:</p></div><div class="myemail-container"><input type="text" value="' + escapeHtml(EMAIL_CONFIG.toEmail) + '" readonly /></div></div><div class="to-container"><div class="to-icon"><p>From:</p></div><div class="myemail-container"><input name="name" type="text" placeholder="Your name" required /></div></div><div class="to-container"><div class="to-icon"><p>Email:</p></div><div class="myemail-container"><input name="email" type="email" placeholder="you@example.com" required /></div></div><div class="to-container"><div class="to-icon"><p>Subject:</p></div><div class="myemail-container"><input name="subject" type="text" placeholder="Quick hello" required /></div><div class="sendmail-icon"><button class="sendmail-img-container" type="submit">Send</button></div></div><textarea class="mail-textarea" name="message" placeholder="Write your message..." required></textarea></form><div class="mail-status">' + escapeHtml(win.data.status || "This form uses the original EmailJS endpoint and falls back to your local mail app if needed.") + "</div></div></div>";
  }

  function renderSecretWindow(win) {
    if (win.data.unlocked) return '<div class="secret-content"><div class="secret-header"><h1 class="secret-title">For my 小豆</h1><p class="secret-date">October 26, 2025</p></div><div class="secret-divider"></div><div class="secret-message"><p class="secret-text">My small bean! You\'re here at Chez Spencer so I created this special place just for you.</p><p class="secret-text">Every day with you is an adventure, and I\'m so grateful to have you in my life.</p><p class="secret-text"><strong>I like you a lot</strong> ♡</p></div><div class="secret-divider"></div><div class="secret-gallery"><h3 class="secret-gallery-title">Our Memories</h3><div class="helper-placeholder-grid">' + IMAGES.map(function (image) { return '<div class="secret-photo"><img src="' + image.src + '" alt="' + escapeHtml(image.caption) + '" /><p class="secret-photo-caption"><strong>' + escapeHtml(image.caption) + '</strong>' + escapeHtml(image.name) + "</p></div>"; }).join("") + '</div></div><div class="secret-divider"></div><div class="secret-footer"><p class="secret-signature">Love always,<br />Wheezy ♡</p></div></div>';
    return '<div class="secret-lock-screen"><div class="secret-lock-content"><div class="secret-lock-icon">🔒</div><h2 class="secret-lock-title">Enter Password</h2><p class="secret-lock-hint">Hint: the original client-side password still works here.</p><form data-secret-form><input class="secret-password-input" name="password" type="password" placeholder="Password..." autofocus />' + (win.data.error ? '<p class="secret-error">' + escapeHtml(win.data.error) + '</p>' : "") + '<button class="secret-submit-btn" type="submit">Unlock</button></form></div></div>';
  }
  ...
  function attachWindowInteractions(win) {
    if (win.type === "computer") {
      ...
      if (form) form.addEventListener("submit", async function (event) {
        ...
        const payload = { service_id: EMAIL_CONFIG.serviceId, template_id: EMAIL_CONFIG.templateId, user_id: EMAIL_CONFIG.publicKey, template_params: { to_email: EMAIL_CONFIG.toEmail, from_name: formData.get("name"), reply_to: formData.get("email"), subject: formData.get("subject"), message: formData.get("message") } };
        win.data.status = "Sending message...";
        updateWindowContent(win);
        try {
          ...
          win.data.status = "Message sent successfully.";
          updateWindowContent(win);
        } catch (error) {
          const subject = encodeURIComponent(String(formData.get("subject") || ""));
          const body = encodeURIComponent("From: " + formData.get("name") + "\\nEmail: " + formData.get("email") + "\\n\\n" + formData.get("message"));
          win.data.status = "Email service unavailable. Opening your local mail app instead.";
          updateWindowContent(win);
  ...
  function initialize() {
    ...
    window.addEventListener("resize", function () {
      state.windows.forEach(function (win) {
        if (win.maximized) { win.width = window.innerWidth - 4; win.height = window.innerHeight - 44; }
        else { win.x = Math.min(win.x, Math.max(0, window.innerWidth - win.width - 10)); win.y = Math.min(win.y, Math.max(0, window.innerHeight - win.height - 52)); }
        win.applyBounds();
      });
    });
  }
